---
title: "ATAC-seq data on chr1:568739-569762"
author: Heejung Shim
output: html_document
---

Here, we will show how to obtain ATAC-seq data on chr1:568739-569762 for three copper treated samples and three media control samples and do data-preprocessing.

First download hdf5 files for all samples from XXX and save them in the following directory:
```{}
cd multiseq_Shim_et_al/data/ATACseq/
```

```{r}
## path to `multiseq_Shim_et_al' repository on your computer
path.multiseq_Shim_et_al ="/Users/hjshim/Documents/Work/github/multiseq_Shim_et_al/"

## set work directory 
setwd(path.multiseq_Shim_et_al)

## Path to directory which contain ATAC-seq data as hdf5 format, 
hdf5.data.path = paste0(path.multiseq_Shim_et_al, "data/ATACseq/")

## load the R scripts to read data from hdf5 files and handle pcr artifacts 
source("scripts/utils.R")

## name for each condition: N702 for copper treatment, N706 for media control, and N705 for ethanol control
name.control = "N706"
name.treatment = "N702"
...
sampling.prob = scan(paste0(hdf5.data.path, "Copper.alt.prob"))
